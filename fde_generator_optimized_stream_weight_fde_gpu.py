...
def generate_document_fde_batch(
    ...
    *,
    ...
    logging.info(f"[FDE Batch] Validation completed in {validation_time:.3f}s")

    projection_time = time.perf_counter()
    use_identity_proj = config.projection_type == ProjectionType.DEFAULT_IDENTITY
    if use_identity_proj:
    ...
    projection_time = time.perf_counter() - projection_time
    logging.info(f"[FDE Batch] Projection completed in {projection_time:.3f}s")
