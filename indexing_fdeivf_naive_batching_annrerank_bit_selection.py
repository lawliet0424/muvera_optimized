...
class ColbertFdeRetrieverNaive:
    def __init__(
        ...
        # Bit selection related attributes
    ...
    def _load_cache(self) -> bool:
        # Try to load structured FDE index with bit selection first
        try:
            # Check if bit selection metadata exists
            bits_path = os.path.join(os.path.dirname(self._fde_path), "selected_bits.npy")
            metadata_path = os.path.join(os.path.dirname(self._fde_path), "bit_selection_metadata.json")
            
            if os.path.exists(bits_path) and os.path.exists(metadata_path):
                logging.info(f"[{self.__class__.__name__}] Loading structured FDE index with bit selection...")
                
                # Load bit selection metadata
                with open(metadata_path, 'r') as f:
                    self.bit_selection_metadata = json.load(f)
                
                # Load compressed FDE index directly
                # Extract P and R from the file path or use metadata
                compressed_path = os.path.join(os.path.dirname(self._fde_path), "compressed_fde_index_3_2.mmap")
                if os.path.exists(compressed_path):
                    # Load compressed FDE index directly
                    compressed_dim = self.bit_selection_metadata['total_compressed_dim']
                    num_docs = self.bit_selection_metadata.get('num_docs', 25657)  # fallback
                    self.fde_index = np.memmap(compressed_path, mode='r', dtype=np.float32, 
                                             shape=(num_docs, compressed_dim))
                    logging.info(f"[{self.__class__.__name__}] Loaded compressed FDE index: {self.fde_index.shape}")
                else:
                    # Fallback to structured loading
                    self.fde_index = load_structured_fde_index(os.path.dirname(self._fde_path), self.bit_selection_metadata)
                ...
                self.bit_selection_metadata = None
                logging.info(f"[{self.__class__.__name__}] Loaded regular FDE index: {self.fde_index.shape}")
                
        except Exception as e:
            logging.warning(f"[{self.__class__.__name__}] Failed to load structured index: {e}, falling back to regular loading")
            self.fde_index = joblib.load(self._fde_path)
            self.bit_selection_metadata = None
            ...
            if need:
                if fde is not None and fde.shape[0] != exp_dim:
                    if not cache_cleared:
                        logging.warning(f"[QueryCache] dim mismatch detected: {fde.shape[0]} vs {exp_dim}. Clearing entire cache.")
                        # Clear entire query cache
                        import shutil
                        if os.path.exists(self._queries_dir):
                            shutil.rmtree(self._queries_dir)
                        os.makedirs(self._queries_dir, exist_ok=True)
                        cache_cleared = True
                        logging.info(f"[QueryCache] Cleared entire query cache due to dimension mismatch")
                ...
                # Apply bit selection to query if metadata is available
                if self.bit_selection_metadata is not None:
                    qfde = apply_bit_selection_to_query(qfde, self.bit_selection_metadata)
                    logging.debug(f"[QueryCache] Applied bit selection to query {qid}: {len(qfde)} dimensions")
...
def ann_aggregator_loop(retriever: ColbertFdeRetrieverNaive,
    ...
    while True:
        ...
        if (qemb is None) or (qfde is None) or (fde := qfde) is None or (fde.shape[0] != exp_dim):
            ...
            # Apply bit selection to query if metadata is available
            if retriever.bit_selection_metadata is not None:
                qfde = apply_bit_selection_to_query(qfde, retriever.bit_selection_metadata)
                logging.debug(f"[ANN] Applied bit selection to query {qid}: {len(qfde)} dimensions")
