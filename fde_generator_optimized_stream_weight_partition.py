...
def _generate_fde_internal(
    ...
    for rep_num in range(config.num_repetitions):
        ...
        # Projection 먼저 수행 (kmeans_gpu와 동일한 순서)
        ...
        # SimHash sketches (projection 후 원본 차원으로 simhash 계산)
    ...
    for rep_num in range(config.num_repetitions):
        ...
        for d in range(num_docs):
            ...
            for b in range(config.num_simhash_projections):
                ...
                if ignore_bit is not None and b == ignore_bit:
                    # ablation: 이 bit을 강제 값으로 설정 (force_bit_value가 지정되면 그 값, 아니면 기본값 0)
                    forced_val = force_bit_value if force_bit_value is not None else 0
                    bit_b = np.full(Ld, forced_val, dtype=np.uint32)
                else:
                    bit_b = bits[:, b]

                p_idx = (p_idx << 1) + (bit_b ^ (p_idx & 1))
