    ...
    for rep_num in range(config.num_repetitions):
        ...
        for d in range(num_docs):
            ...
            if config.fill_empty_partitions and (~nz).any():
                ...
                # 각 empty partition에 대해 마스킹된 점들의 각 차원별 최댓값 계산 (max pooling)
                # Pts는 [Ld, proj_dim], mask는 [E, Ld]
                # 브로드캐스트: [E, Ld, proj_dim]
                # 마스킹된 점들은 원래 값, 마스킹되지 않은 점들은 -inf로 설정
                masked_pts = np.where(mask[:, :, None], Pts[None, :, :], -np.inf)  # [E, Ld, proj_dim]
                # 각 차원별 최댓값 계산 (axis=1: Ld 차원에 대해 max pooling)
                fill_values = np.max(masked_pts, axis=1)  # [E, proj_dim]
                # -inf가 남아있는 경우 (마스킹된 점이 없는 경우) 0으로 설정
                fill_values = np.where(np.isinf(fill_values), 0.0, fill_values)  # [E, proj_dim]
