...
class ColbertFdeRetrieverNaive:
    def __init__(
        ...
        self.out_path = os.path.join(self._cache_dir, faiss_default)
    ...
    def _load_cache(self) -> bool:
        self.fde_index = joblib.load(self.in_path)
        ...
        # FAISS 로드 & 차원 검증
        if self.use_faiss_ann and os.path.exists(self.in_path):
            try:
                self.faiss_index = faiss.read_index(self.in_path)
    ...
    def _build_or_load_faiss_index(self):
        if not self.use_faiss_ann:
            return
        if self.faiss_index is not None and os.path.exists(self.in_path):
            return
        ...
        faiss.write_index(index, self.out_path)
...
if __name__ == "__main__":
    ...
    #in_path = pathlib.Path(args.input or in_default)
    #out_path = pathlib.Path(args.output or faiss_default)
