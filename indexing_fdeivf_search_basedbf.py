...
# 데이터셋 경로
dataset = "quora" # fiqa, arguana, scidocs, treccovid, quora
    ...
    try:
        if not os.path.exists(_per_experiment_log_path):
            with open(_per_experiment_log_path, "a", encoding="utf-8") as f:
                f.write(f"Dataset: {DATASET_REPO_ID}, Queries: {len(queries)}, FirstCand: {FAISS_CANDIDATES} | "
                        ...
                        f"Recall@{TOP_K}: {bf_recall:.4f}, nDCG@{TOP_K}: {bf_ndcg:.4f}\n"
                        )
        else:
            with open(_per_experiment_log_path, "a", encoding="utf-8") as f:
                f.write(f"Dataset: {DATASET_REPO_ID}, Queries: {len(queries)}, FirstCand: {FAISS_CANDIDATES} | "
