...
# FDE 구현 (업로드된 파일 사용)
from fde_generator_optimized_stream_pca_gpu_patched import (
    FixedDimensionalEncodingConfig,
    ProjectionType,
    generate_query_fde,
    generate_document_fde_batch,
    set_pca_projection_matrices,
    clear_pca_projection_matrices,
)
...
TOP_K = 10
FILENAME = "main_weight_pca_gpu_patched"
...
class ColbertFdeRetriever:
    ...
    def _cache_exists(self) -> bool:
        pca_path = os.path.join(self._cache_dir, "pca_centers.pkl")
    ...
    def _save_cache(self):
        ...
        if self.pca_centers is not None:
            pca_path = os.path.join(self._cache_dir, "pca_centers.pkl")
            joblib.dump(self.pca_centers, pca_path)
            ...
            json.dump(
                {
                    ...
                    "config": {
                        ...
                        "use_pca_partition": self.doc_config.use_pca_partition,
    ...
    def _load_cache(self) -> bool:
        ...
        if os.path.exists(pca_path):
            self.pca_centers = joblib.load(pca_path)
    ...
    def index(self, corpus: dict):
        ...
        #[1017] PCA partition별 indice별 원소 개수 csv 파일 저장 필요------------------------------------
        partition_count_dir = os.path.join(QUERY_SEARCH_DIR, f"rep{args.rep}_pca{args.simhash}_rerank{args.rerank}")
        ...
        # Top-10 dimension만 사용하므로 projection_dimension은 10으로 고정
        ...
        logging.info(f"[PCA] Using top-20 dimensions for partition assignment")
        ...
        for batch_start in range(0, len(self.doc_ids), ATOMIC_BATCH_SIZE):
            ...
            # ---- PCA projection (replace random AMS) ----
            # Train PCA projection matrices once (using first loaded batch as sample) and
            # register them so both document FDE and query FDE use the same projection.
            if (
                not hasattr(self, "_pca_proj_mats")
                and self.doc_config.projection_type == ProjectionType.AMS_SKETCH
                ):
                try:
                    rng = np.random.default_rng(self.doc_config.seed)
                    max_points = 200000  # cap for stability
                    pts_list = []
                    total_pts = 0

                    # sample tokens from the first loaded batch_embeddings
                    for emb in batch_embeddings:
                        ...
                        if L == 0:
                            continue
                        take = max(1, int(L * 0.10))  # 10% token sampling per doc
                        take = min(take, L)
                        idxs = rng.choice(L, size=take, replace=False)
                        pts_list.append(emb[idxs])
                        total_pts += take
                        if total_pts >= max_points:
                            break

                    if len(pts_list) == 0:
                        raise RuntimeError("No points sampled for PCA pretraining.")

                    sampled_points = np.vstack(pts_list)
                    projection_dim = int(self.doc_config.projection_dimension or D)

                    pca_mats = []
                    for rep_num in range(self.doc_config.num_repetitions):
                        pca = PCA(n_components=projection_dim, svd_solver="randomized", random_state=self.doc_config.seed + rep_num)
                        pca.fit(sampled_points)

                        # projection matrix: (D, P)
                        proj_mat = pca.components_.T.astype(np.float32, copy=False)
                        pca_mats.append(proj_mat)

                        # top-10 important ORIGINAL dims (weighted abs loadings)
                        evr = pca.explained_variance_ratio_
                        comps = pca.components_
                        dim_scores = (np.abs(comps).T * evr).sum(axis=1)
                        top10 = np.argsort(-dim_scores)[:10]

                        out_path = os.path.join(self._cache_dir, f"pca_top10_dims_rep{rep_num}.txt")
                        with open(out_path, "w", encoding="utf-8") as f:
                            f.write("# top-10 ORIGINAL feature dims by PCA weighted abs-loadings\n")
                            f.write(f"# seed={self.doc_config.seed}, rep={rep_num}, D={D}, P={projection_dim}\n")
                            f.write("# rank\tfeature_dim\tscore\n")
                            for rank, d0 in enumerate(top10, start=1):
                                f.write(f"{rank}\t{int(d0)}\t{float(dim_scores[d0]):.10f}\n")

                        logging.info(f"[PCA] rep={rep_num} top-10 dims saved: {out_path}")

                    set_pca_projection_matrices(pca_mats)
                    self._pca_proj_mats = pca_mats
                    logging.info(f"[PCA] Registered PCA projection matrices: reps={len(pca_mats)}, shape={pca_mats[0].shape}")
                except Exception as e:
                    logging.warning(f"[PCA] PCA pretraining skipped (fallback to random AMS). Reason: {e}")
            # -------------------------------------------
            batch_fde_result = generate_document_fde_batch(
                batch_embeddings,
                self.doc_config,
                query_or_doc=False,  # document FDE 생성
            ...
            if isinstance(batch_fde_result, tuple):
                if len(batch_fde_result) == 4:
                    batch_fde, partition_counter, pca_centers_batch, top10_indices_list = batch_fde_result
                else:
                    batch_fde, partition_counter, pca_centers_batch = batch_fde_result
                    top10_indices_list = None
                
                # 첫 번째 배치에서 PCA centers와 top-20 indices 저장
                if batch_start == 0:
                    if pca_centers_batch is not None:
                        self.pca_centers = pca_centers_batch
                        logging.info(f"[PCA] Saved PCA centers from first batch: {len(pca_centers_batch)} repetitions")
                    if top10_indices_list is not None:
                        # PCA projection matrices에 top-20 indices 등록
                        from fde_generator_optimized_stream_pca_gpu_patched import set_pca_projection_matrices
                        if hasattr(self, '_pca_proj_mats') and self._pca_proj_mats is not None:
                            set_pca_projection_matrices(self._pca_proj_mats, top10_indices_list)
                            logging.info(f"[PCA] Registered top-20 indices: {len(top10_indices_list)} repetitions")
            else:
            ...
            # Step 6: PCA 통계 저장
            if partition_counter is not None and isinstance(partition_counter, dict):
                for doc_idx, counter_array in partition_counter.items():
                    global_doc_idx = batch_start + doc_idx
                    for rep_num in range(counter_array.shape[0]):
                        for partition_idx in range(counter_array.shape[1]):
                            count = counter_array[rep_num, partition_idx]
                            if count > 0:  # 0이 아닌 경우만 저장
    ...
    def precompute_queries(self, queries: dict):
        ...
        for qid, qtext in queries.items():
            ...
            query_fde_result = generate_query_fde(query_embeddings, query_config, True, self.pca_centers)
...
if __name__ == "__main__":
    ...
    retrievers = {
        "2. ColBERT + FDE (+Chamfer rerank)": ColbertFdeRetriever(
            ...
            latency_log_path=os.path.join(QUERY_SEARCH_DIR, f"rep{args.rep}_pca{args.simhash}_rerank{args.rerank}", "latency.tsv"),  # QID\tSearch\tRerank
    ...
    for name, retriever in retrievers.items():
        ...
        # 지연시간 로그 파일 초기화
        latency_dir = os.path.join(QUERY_SEARCH_DIR, f"rep{args.rep}_pca{args.simhash}_rerank{args.rerank}_proj{args.projection}")
