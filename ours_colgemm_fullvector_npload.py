...
class TPackReader:
    def __init__(self, tpack_dir: str):
        ...
        # >>> NEW: memmap 전체(B^T = (D, W))를 F-order로 맵핑
        # np.memmap은 헤더가 없는 순수 바이너리에도 동작.
        self._mm = np.memmap(self.tokensF_path, dtype=np.float32, mode="r",
                             shape=(self.D, self.total_cols), order="F")
        
        # ==== NEW: O_DIRECT fd (가능하면) ====
        self._fd_direct = None
        try:
            O_DIRECT = getattr(os, "O_DIRECT", 0x4000)  # 리눅스 값; 없으면 예외 없이 실패시 폴백
            self._fd_direct = os.open(self.tokensF_path, os.O_RDONLY | O_DIRECT)
        except Exception:
            self._fd_direct = None  # 폴백: 사용할 수 없으면 None

        # libc 심볼 준비 (posix_memalign / pread / posix_fadvise)
        self._libc = None
        try:
    ...
    # >>> NEW: np.load와 유사한 경로 — OS 페이지 캐시에 의존, 복사/시스템콜 최소화
    def memmap_cols(self, start_col: int, n_cols: int) -> np.ndarray:
        if n_cols <= 0:
            return np.empty((self.D, 0), dtype=np.float32, order="F")
        # memmap으로 전체를 이미 (D, W)로 잡아두었으므로, 열 슬라이스는 즉시 뷰 반환
        # 반환 shape: (D, n_cols), F-order (뷰)
        return self._mm[:, start_col:start_col + n_cols]
    
    # ==== NEW: pread + fadvise(DONTNEED) => 항상 I/O 타고 곧바로 캐시에서 제거 ====
    def pread_cols_nocache(self, start_col: int, n_cols: int) -> np.ndarray:
        if n_cols <= 0:
            return np.empty((self.D, 0), dtype=np.float32, order="F")
        byte_off = start_col * self.D * self.itemsize
        byte_len = n_cols * self.D * self.itemsize

        buf = os.pread(self.fd, byte_len, byte_off)  # 여기서 I/O (캐시히트 가능성은 있음)
        # 읽은 범위를 즉시 캐시에서 드랍 (가능한 플랫폼에서)
        if self._libc is not None and hasattr(self._libc, "posix_fadvise"):
            POSIX_FADV_DONTNEED = 4
            # off_t와 size_t 시그니처로 호출
            self._libc.posix_fadvise.argtypes = [ctypes.c_int, ctypes.c_longlong, ctypes.c_longlong, ctypes.c_int]
            self._libc.posix_fadvise(self.fd, ctypes.c_longlong(byte_off),
                                     ctypes.c_longlong(byte_len), ctypes.c_int(POSIX_FADV_DONTNEED))

        arr = np.frombuffer(buf, dtype=np.float32, count=n_cols * self.D)
        # 버퍼 생명주기/정렬 이슈 피하려면 한 번 복사해서 안전한 F-contig 배열로 반환
        out = np.array(arr, dtype=np.float32, copy=True).reshape((self.D, n_cols), order="F")
        return out

    # ==== NEW: O_DIRECT + aligned pread (가능하면), 실패 시 pread_cols_nocache로 폴백 ====
    def direct_cols(self, start_col: int, n_cols: int) -> np.ndarray:
        if n_cols <= 0:
            return np.empty((self.D, 0), dtype=np.float32, order="F")
        if self._fd_direct is None or self._libc is None:
            # O_DIRECT 불가 → no-cache 경로로 폴백
            return self.pread_cols_nocache(start_col, n_cols)

        # 요청 바이트 범위
        byte_off = start_col * self.D * self.itemsize
        byte_len = n_cols * self.D * self.itemsize

        # O_DIRECT 제약: offset/length/버퍼가 보통 4096 정렬 필요
        ALIGN = 4096
        aligned_off = (byte_off // ALIGN) * ALIGN
        head_skip = byte_off - aligned_off
        aligned_len = ((head_skip + byte_len + (ALIGN - 1)) // ALIGN) * ALIGN

        # posix_memalign으로 정렬 버퍼 확보
        void_pp = ctypes.c_void_p()
        rc = self._libc.posix_memalign(ctypes.byref(void_pp), ALIGN, aligned_len)
        if rc != 0 or not void_pp.value:
            # 정렬 확보 실패 → no-cache 폴백
            return self.pread_cols_nocache(start_col, n_cols)

        # libc.pread로 O_DIRECT fd에서 읽기
        self._libc.pread.argtypes = [ctypes.c_int, ctypes.c_void_p, ctypes.c_size_t, ctypes.c_longlong]
        nread = self._libc.pread(self._fd_direct, void_pp, ctypes.c_size_t(aligned_len), ctypes.c_longlong(aligned_off))
        if nread < head_skip + byte_len:
            # 충분히 못 읽었으면 폴백
            # (할당 해제는 libc free로)
            if hasattr(self._libc, "free"):
                self._libc.free(void_pp)
            return self.pread_cols_nocache(start_col, n_cols)

        # 정렬 버퍼 → 우리가 원하는 서브 슬라이스를 안전 복사
        raw_type = ctypes.c_char * aligned_len
        raw_buf = raw_type.from_address(void_pp.value)
        # 필요한 부분만 잘라서 바이트 복사 (버퍼 생명주기 분리)
        view = memoryview(raw_buf)[head_skip:head_skip + byte_len]
        arr = np.frombuffer(view.tobytes(), dtype=np.float32, count=n_cols * self.D)

        # 정렬 버퍼 해제
        if hasattr(self._libc, "free"):
            self._libc.free(void_pp)

        out = np.array(arr, dtype=np.float32, copy=False).reshape((self.D, n_cols), order="F")
        return out
    ...
    if retriever.tpack is not None:
        ...
        if int(tp.D) != Dq:
            ...
            for did, s, e in doc_spans:
                ...
                # Bblk = tp.memmap_cols(s, ncols)
                Bblk = tp.pread_cols_nocache(s, ncols)
                # 즉시 GEMM
...
def ndcg_at_k_wrt_bf(results_topk: Dict[str, List[str]], bf_truth: Dict[str, List[Tuple[str, float]]], k: int) -> float:
    def _discount(i: int) -> float: return 1.0 / math.log2(i + 1)
    per_q = []
...
if __name__ == "__main__":
    ...
    try:
        ...
        logging.warning(f"Failed to write per-experiment row: {e}")